			...
			if ([[self visibleTableView] numberOfSections] > sectionIndex) {
				...
				if (numberOfRowsInSection > row) {
					if (numberOfRowsInSection != [section numberOfRowsInSection]) {
						/**
						 **	It appears to me that this is happening because a row moves from section 3 to section 0
						 ** We reload the 0th section, and that cause the table to interrogate the past-week section and finds that a cell is missing
						 **	This is causing a crash.
						 ** My solution is to reload the whole table when we encounter a row-count mismatch
						 **	This will cause an unnecessary table reload when a new row arrives, however
						 ** the more common case of a row jumping to a different section does require the reload to prevent the crash
						 */
						[[self visibleTableView] reloadData];
					} else {
